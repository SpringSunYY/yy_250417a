<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="编号" prop="balanceId">
        <el-input
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <!--          size="mini"-->
      <!--          @click="handleAdd"-->
      <!--          v-hasPermi="['manage:userBalanceInfo:add']"-->
      <!--        >新增-->
      <!--        </el-button>-->
      <!--      </el-col>-->
      <!--      <el-col :span="1.5">-->
      ...
      <!--          :disabled="single"-->
      <!--          @click="handleUpdate"-->
      <!--          v-hasPermi="['manage:userBalanceInfo:edit']"-->
      <!--        >修改-->
      <!--        </el-button>-->
      <!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['manage:userBalanceInfo:remove']"
        >删除
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['manage:userBalanceInfo:export']"
        >导出
        </el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="userBalanceInfoList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center"/>
      <el-table-column label="编号" align="center" v-if="columns[0].visible" prop="balanceId"/>
...
} from '@/api/manage/userBalanceInfo'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      listUserBalanceInfo(this.queryParams).then(response => {
        this.userBalanceInfoList = response.rows
    ...
    handleUpdate(row) {
      this.reset()
      const balanceId = row.balanceId || this.ids
      getUserBalanceInfo(balanceId).then(response => {
    ...
    submitForm() {
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.form.balanceId != null) {
            updateUserBalanceInfo(this.form).then(response => {
              this.$modal.msgSuccess('修改成功')
              this.open = false
              this.getList()
            })
          } else {
            addUserBalanceInfo(this.form).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const balanceIds = row.balanceId || this.ids
      this.$modal.confirm('是否确认删除用户余额编号为"' + balanceIds + '"的数据项？').then(function() {
        return delUserBalanceInfo(balanceIds)
      }).then(() => {
        this.getList()
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('manage/userBalanceInfo/export', {
        ...this.queryParams
      }, `userBalanceInfo_${new Date().getTime()}.xlsx`)
    }
  }
}
</script>
