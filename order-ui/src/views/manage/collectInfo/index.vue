<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="编号" prop="collectId">
        <el-input
...
<script>
import { listCollectInfo, getCollectInfo, delCollectInfo, addCollectInfo, updateCollectInfo } from "@/api/manage/collectInfo";
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getCollectInfo(collectId).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除商品收藏编号为"' + collectIds + '"的数据项？').then(function() {
        return delCollectInfo(collectIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('manage/collectInfo/export', {
        ...this.queryParams
